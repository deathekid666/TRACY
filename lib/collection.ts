...
export async function collectPublicSources(caseId:string,query:string,mode:"quick"|"deep"="deep"){
  ...
  const providerStatus=serperCalls>0&&failedSerperCalls>=serperCalls
    ?"unavailable"
    :(failedSerperCalls>0?"degraded":"healthy");
  const eventTitle=providerStatus==="unavailable"
    ?"Public-footprint discovery unavailable"
    :"Deep public-footprint discovery";
  const description=providerStatus==="unavailable"
    ?`${mode==="quick"?"Quick":"Deep"} scan could not run fresh Google/Serper discovery because all ${serperCalls} search calls failed. Existing retained evidence was left intact; this is a provider failure, not a zero-results finding.`
    :`${mode==="quick"?"Quick":"Deep"} scan used ${serperCalls} rate-limited public-web searches; ${failedSerperCalls} calls failed after retries. Preserved ${added} identity-supported sources, verified ${deepValidated} names inside fetched documents/pages, rejected ${deepRejected} deep candidates and filtered ${noise} unrelated results. Removed ${staleIds.length} stale unverified sources.`;
