...
export default async function MediaPage({params}:{params:Promise<{id:string}>}){
 ...
 if(!c) notFound();
 const media=c.sources.flatMap(source=>{const m=meta(source.metadata);const image=typeof m.imageUrl==="string"?m.imageUrl:"";const decision=String(m.curatedDecision??"UNREVIEWED");return image&&decision!=="REJECT"?[{source,image,publishedAt:m.publishedAt}]:[]});
