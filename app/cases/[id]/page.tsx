      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-[1.45fr_.75fr]">
        <div className="space-y-6">
          ...
          {(reviewSources.length>0||rejectedSources.length>0)&&<section className="rounded-2xl border border-amber-400/15 bg-slate-950/55 p-5">
            <div className="flex flex-wrap items-center justify-between gap-3"><div><div className="flex items-center gap-2"><Database className="h-4 w-4 text-amber-300"/><h2 className="font-medium">Candidate findings</h2></div><p className="mt-1 text-xs text-slate-500">Only plausible candidates are shown on the dossier. Low-confidence raw search results remain available in Report and Sources for audit.</p></div><Link href={`/cases/${id}/report`} className="text-xs text-cyan-300">Open full report →</Link></div>
            <div className="mt-4 grid gap-3 md:grid-cols-2">{reviewSources.slice(0,12).map(s=>{const d=decision(s);const m=meta(s.metadata);return <a key={s.id} href={s.url} target="_blank" rel="noreferrer" className="rounded-xl border border-slate-800 bg-slate-900/30 p-3 hover:border-amber-400/20"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="line-clamp-2 text-sm">{s.title||s.url}</div><div className="mt-1 text-[10px] text-slate-600">{s.provider||"SOURCE"} · {sourceCategory(s).replaceAll("_"," ")}</div></div><span className="shrink-0 rounded-full border border-amber-500/30 px-2 py-1 text-[9px] uppercase tracking-wider text-amber-300">{d}</span></div>{text(m.curatedReason)&&<div className="mt-2 line-clamp-2 text-[10px] text-slate-500">{text(m.curatedReason)}</div>}</a>})}</div>
            {reviewSources.length>12&&<div className="mt-4 text-xs text-slate-500">+ {reviewSources.length-12} more plausible candidates in the full report.</div>}
            {rejectedSources.length>0&&<div className="mt-3 text-xs text-slate-600">{rejectedSources.length} low-confidence/raw results are preserved in Sources and the complete report register.</div>}
          </section>}
